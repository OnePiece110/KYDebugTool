...
final class KYViewElement: NSObject, KYElement {
    var label: KYElementLabel {
        guard let view = view else {
            return KYElementLabel(name: nil)
        }
        if let viewController = getViewController(view: view) {
            ...
            return KYElementLabel(name: name, classification: .important)
        } else {
            return KYElementLabel(name: String(describing: Swift.type(of: view)))
        }
    }
    ...
    var children: [KYElement] {
        guard let view = view else {
            return []
        }
        return view.subviews.map { KYViewElement(view: $0) }
    }
