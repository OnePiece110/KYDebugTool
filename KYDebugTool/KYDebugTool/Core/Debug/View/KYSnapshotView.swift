...
protocol KYSnapshotViewDelegate: AnyObject {
...
/// hierarchy snapshot.
final class KYSnapshotView: UIView {
    weak var delegate: KYSnapshotViewDelegate?
    ...
    private lazy var depthSlider = KYRangeSlider()
    private let descriptionLabel = UILabel()

    private var snapshotIdentifierToNodesMap = [String: KYSnapshotNodes]()
    ...
    private var highlightedNodes: KYSnapshotNodes?
    ...
    @objc private func depthSliderChanged(sender: KYRangeSlider) {
...
/// snapshot.
private final class KYSnapshotNodes {
    let snapshot: KYSnapshot
    let depth: Int
    ...
    init(snapshot: KYSnapshot, depth: Int) {
        self.snapshot = snapshot
        self.depth = depth
    }
}
...
private func highlightNode(snapshot: KYSnapshot, color: UIColor) -> SCNNode {
...
private func snapshotNode(snapshot: KYSnapshot,
                          parentSnapshot: KYSnapshot?,
                          rootNode: SCNNode,
                          parentSnapshotNode: SCNNode?,
                          depth: inout Int,
                          snapshotIdentifierToNodesMap: inout [String: KYSnapshotNodes],
                          configuration: KYSnapshotViewConfiguration,
    ...
    let nodes = KYSnapshotNodes(snapshot: snapshot, depth: depth)
    ...
    let headerAttributes: KYSnapshotViewConfiguration.HeaderAttributes
...
private func snapshotNode(snapshot: KYSnapshot) -> SCNNode {
    let path = UIBezierPath(rect: CGRect(origin: .zero, size: snapshot.frame.size))
    let shape = SCNShape(path: path, extrusionDepth: .zero)
    let material = SCNMaterial()
    material.isDoubleSided = true
...
private func headerNode(snapshot: KYSnapshot,
                        attributes: KYSnapshotViewConfiguration.HeaderAttributes) -> SCNNode? {
...
private func nameTextGeometry(label: KYElementLabel, font: UIFont) -> SCNText? {
